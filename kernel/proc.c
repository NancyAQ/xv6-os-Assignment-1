...
int
wait(uint64 addr,uint64 msg_p) //task 3 added msg pointer
{
  struct proc *pp;
  int havekids, pid;
  struct proc *p = myproc();
    ...
    for(pp = proc; pp < &proc[NPROC]; pp++){
      if(pp->parent == p){
        ...
        if(pp->state == ZOMBIE){
          ...
          //task 3 added copyout and same condition as above
          if( msg_p!=0 && copyout(p->pagetable, msg_p, (char *)&pp->exit_msg, 
...
scheduler(void)
{
  ...
  for(;;){
    ...
    for(p = proc; p < &proc[NPROC]; p++) {
      ...
      if(p->state == RUNNABLE && (p->affinity_mask==0 ||(bin_rep & p->effective_affinity_mask))) { //if for task 6
...
sched(void)
{
  ...
  intena = mycpu()->intena;
  //start of task 6 code to yield cpu
  push_off(); //for task 6
  int cpu_id=cpuid(); 
  pop_off();
  int bin_rep=1;
...
int set_affinity_mask(int mask_size){
  ...
  myproc()->effective_affinity_mask=mask_size; //task6
  // release(&myproc()->lock);
  // pop_off();
  return 0;
